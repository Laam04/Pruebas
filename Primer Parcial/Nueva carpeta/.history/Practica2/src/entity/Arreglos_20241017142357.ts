...
async function migrateData() {
  ...
  // Migrar Vehículos
  for (const vehiculo of data.vehiculo.datos) {
    await connection.execute(
      'INSERT INTO Vehiculo (ID, Descripcion, Placa, Color) VALUES (?, ?, ?, ?)', 
      [vehiculo.ID, vehiculo.Descripcion, vehiculo.Placa, vehiculo.Color]
    );
  }

  // Migrar Espacios de Parqueo
  for (const espacio of data.espacioParqueo.datos) {
    await connection.execute(
      'INSERT INTO EspacioParqueo (ID, Descripcion) VALUES (?, ?)', 
      [espacio.ID, espacio.Descripcion]
    );
  }

  // Migrar Parquear (transacciones)
  for (const transaccion of data.parquear.datos) {
    await connection.execute(
      `INSERT INTO Parquear (ID, ID_Vehiculo, ID_Parqueo, FechaHoraEntrada, FechaHoraSalida) 
       VALUES (?, ?, ?, ?, ?)`,
      [
        transaccion.ID, 
